...
When specified, retrieves app protection policies from the CIPP report database cache instead of live Microsoft Graph data. Use 'AllTenants' with this switch for cached cross-tenant data.
 ...
 #### EXAMPLE 1
```powershell
PS > Get-CIPPAppProtectionPolicies -CustomerTenantID "contoso.onmicrosoft.com"
```
 #### EXAMPLE 2
```powershell
PS > Get-CIPPAppProtectionPolicies -CustomerTenantID "contoso.onmicrosoft.com" -UseReportDB
```
