...
When specified, retrieves compliance policies from the CIPP report database cache instead of live Microsoft Graph data. Use 'AllTenants' with this switch for cached cross-tenant data.
 ...
 #### EXAMPLE 1
```powershell
PS > Get-CIPPCompliancePolicies -CustomerTenantID "contoso.onmicrosoft.com"
```
 #### EXAMPLE 2
```powershell
PS > Get-CIPPCompliancePolicies -CustomerTenantID "contoso.onmicrosoft.com" -UseReportDB
```
